...
for (ii in 1:N) {
  tryCatch({
    ...
    xlin <- runif(n)
    y <- beta0 + beta1 * xlin + fsmooth + ysp + er
    ymean <- beta0 + beta1 * xlin + fsmooth + ysp
    ...
    ymean.0 <- beta0 + beta1 * xlin.0 + fsm(xgrid) + ysp.0
    er.0 <- rnorm(n, mean = 0, sd = sqrt(sigma2))
    y0 <- ymean.0 + er.0
    ...
    fitted.smooth   <- inla.mod$summary.fitted.values$mean[index_pred.smooth]
    fitted.smooth.lowerci <- inla.mod$summary.fitted.values$`0.025quant`[index_pred.smooth]
    fitted.smooth.upperci <- inla.mod$summary.fitted.values$`0.975quant`[index_pred.smooth]
    ...
    lower.y0 <- apply(pred.samples, 2, quantile, probs = 0.025)
    upper.y0 <- apply(pred.samples, 2, quantile, probs = 0.975)
